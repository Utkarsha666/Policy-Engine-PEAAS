...
public class CEORegistration extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            int i = db.ceoData(name, address, email, city, username, pwd);

            if (i != 0) {

                session.setAttribute("reg", name + " Your Registration Successfull!!!"); //Save an attribute in session
                rd = request.getRequestDispatcher("OwnerHome.jsp"); //attribute is sent to index.jsp
            ...
            Logger.getLogger(CEORegistration.class.getName()).log(Level.SEVERE, null, ex);
