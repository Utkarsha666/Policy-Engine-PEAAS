...
// the function receives plaintext and key as an argument
public static String encrypt(String plaintext, String key)throws NoSuchAlgorithmException, NoSuchPaddingException,InvalidKeyException, IllegalBlockSizeException,BadPaddingException, IOException{
	byte[] raw = DatatypeConverter.parseHexBinary(key); //convert to hex
	SecretKeySpec skeySpec = new SecretKeySpec(raw, "AES"); //key in AES form
	Cipher cipher = Cipher.getInstance(ALGORITMO);  //Cipher text
